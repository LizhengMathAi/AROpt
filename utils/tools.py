...
def adjust_learning_rate(optimizer, epoch, args):
    if args.lradj == 'type1':
        # lr_adjust = {epoch: args.learning_rate * (0.5 ** ((epoch - 1) // 1))}  # original decay is 0.5
...
def autoregressive_forcast(model, outputs, batch_y, batch_y_mark, f_dim, num_ar, args):
    ...
    :returns: generated multi-step ahead forecast
    """
...
def autoregressive_criterion(model, outputs, batch_y, batch_y_mark, f_dim, args):
    """
    Compute loss for autoregressive training pipeline
    :param model: the iTransformer/iInformer/iFlowformer/iFlashformer model
    ...
    for k in range(1, num_ar):
        pre_error = criterion(seq_outputs[k-1], seq_batch_y[k-1]).detach()
        error = criterion(seq_outputs[k], seq_batch_y[k])
        penalty = torch.abs(error- pre_error)
        loss = loss + args.gamma**k * ((1-args.beta) * error + args.beta * penalty)
